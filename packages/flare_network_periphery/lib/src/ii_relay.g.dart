...
// Contract: IIRelay
// Functions: 17 — 13 readable via eth_call, 5 requiring a
...
/// Typed bindings for Flare's `IIRelay` contract.
///
...
class IIRelayContract {
  ...
  /// ABI descriptor for `setSigningPolicy((uint24,uint32,uint16,uint256,address[],uint16[]))`.
  static final AbiFunction setSigningPolicyFn = AbiFunction(
    name: 'setSigningPolicy',
    inputs: [
      AbiParameter(
        name: '_signingPolicy',
        type: AbiType.parse(
          '(uint24,uint32,uint16,uint256,address[],uint16[])',
        ),
      ),
    ],
  ...
  /// Builds an unsigned `setSigningPolicy((uint24,uint32,uint16,uint256,address[],uint16[]))`
  /// transaction.
  ///
  /// Declared `nonpayable` in Solidity, so it changes state and
  /// must be signed. This package holds no keys: pass the
  ...
  /// gas and fees, then hand
  /// [TransactionRequest.toWalletJson] to a wallet.
  TransactionRequest setSigningPolicyTx(
    List<Object?> signingPolicy, {
    EthAddress? from,
  }) => TransactionRequest.callFunction(
    to: address,
    function: setSigningPolicyFn,
    args: [signingPolicy],
    from: from,
  );
