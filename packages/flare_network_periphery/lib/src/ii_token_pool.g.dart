...
/// Typed bindings for Flare's `IITokenPool` contract.
///
...
class IITokenPoolContract {
  ...
  const IITokenPoolContract({required this.client, required this.address});

  /// Resolves `IITokenPool` through the [ContractRegistry].
  ///
  /// This contract has no entry in Flare's published products
  /// map, so [registryName] is required — there is no name that
  /// could be defaulted correctly. Call `ContractRegistry.listAll`
  /// to see what this network registers.
  static Future<IITokenPoolContract> resolve(
    FlareClient client, {
    ...
    return IITokenPoolContract(client: client, address: resolved);
  }
