...
// Contract: IFtsoRegistry
// Functions: 19 readable of 19 total (state-changing functions are omitted — this SDK does not sign).
//
// Regenerate with:
...
/// Typed read bindings for Flare's `IFtsoRegistry` contract.
///
/// Resolve it through the registry rather than hardcoding an
/// address — Flare redeploys contracts.
class IFtsoRegistryContract {
  ...
  const IFtsoRegistryContract({required this.client, required this.address});

  /// Resolves `IFtsoRegistry` through the [ContractRegistry].
  ///
  /// Registered as `FtsoRegistry`, which is what the registry
  /// answers to — the Solidity interface name is not a registry
  /// key.
  static Future<IFtsoRegistryContract> resolve(
    FlareClient client, {
    ContractRegistry? registry,
    String registryName = 'FtsoRegistry',
  }) async {
    final resolved = await (registry ?? ContractRegistry(client)).addressOf(
      registryName,
    );
    return IFtsoRegistryContract(client: client, address: resolved);
  }
