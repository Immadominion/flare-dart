...
// Contract: IICleanupBlockNumberManager
...
/// Typed bindings for Flare's `IICleanupBlockNumberManager` contract.
///
...
class IICleanupBlockNumberManagerContract {
  ...
  const IICleanupBlockNumberManagerContract({
    required this.client,
    required this.address,
  });

  /// Resolves `IICleanupBlockNumberManager` through the [ContractRegistry].
  ///
  /// This contract has no entry in Flare's published products
  /// map, so [registryName] is required — there is no name that
  /// could be defaulted correctly. Call `ContractRegistry.listAll`
  /// to see what this network registers.
  static Future<IICleanupBlockNumberManagerContract> resolve(
    FlareClient client, {
    ...
    return IICleanupBlockNumberManagerContract(
      client: client,
      address: resolved,
    );
  }
