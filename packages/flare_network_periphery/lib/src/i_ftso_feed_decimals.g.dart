...
class IFtsoFeedDecimalsContract {
  ...
  const IFtsoFeedDecimalsContract({required this.client, required this.address});
  ...
  static final AbiFunction getCurrentDecimalsBulkFn = AbiFunction(
    ...
    outputs: [
      AbiParameter(name: '_decimals', type: AbiType.parse('bytes')),
    ],
    stateMutability: StateMutability.view,
  );
  ...
  ///
  /// Declared `view` in Solidity; read via `eth_call`.
  Future<Uint8List> getDecimalsBulk(Uint8List feedIds, BigInt rewardEpochId) async {
    final out = await client.callFunction(
      contract: address,
  ...
  ///
  /// Declared `view` in Solidity; read via `eth_call`.
  Future<({List<BigInt> decimals, List<BigInt> validFromEpochId, List<bool> fixed})> getScheduledDecimalsChanges(Uint8List feedId) async {
    final out = await client.callFunction(
      contract: address,
      function: getScheduledDecimalsChangesFn,
      args: [feedId],
    );
    return (decimals: (out[0]! as List).cast<BigInt>(), validFromEpochId: (out[1]! as List).cast<BigInt>(), fixed: (out[2]! as List).cast<bool>());
  }
